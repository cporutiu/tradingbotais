...
10. Exit a sector after 2 consecutive failed trades — **applies only to thesis/R:R failures**: a candidate that fails the R:R/entry checklist, or a position closed on a confirmed thesis break. A mechanical trailing-stop exit where the underlying thesis remained intact does NOT count toward this rule. Clarified 2026-08-08 after AMD/NVDA's Jul 27/28 stop-outs (sector-wide semiconductor rotation, both theses intact) were read as "2 failed trades" and blocked NVDA re-entry for over a week — that reading no longer applies.
...
16. **Candidate pool widening protocol:** If the same sector or instrument type fails the R:R/entry checklist for 3+ consecutive research sessions, widen the search along two axes rather than re-testing more of the same type:
    - **Sector breadth:** pull in RS-ranked sectors not currently on the watchlist. All of Financials (XLF), Healthcare (XLV), Consumer Discretionary (XLY), Consumer Staples (XLP), Communication Services (XLC), Real Estate (XLRE), and Utilities (XLU) are already tracked by the Benzinga/Congress scanners (`scripts/fetch_benzinga.py` / `scripts/fetch_congress.py` UNIVERSE) but had zero trade candidates sourced from them across the 2026-06-08 to 2026-08-08 window.
    - **Instrument type:** default to sourcing 1-2 single-stock candidates per sector before falling back to the sector ETF. Single stocks carry analyst price targets that can clear the R:R checklist; a sector ETF sitting near its 52-week high structurally cannot pass a 10%-stop R:R test against nearby chart resistance — this is a math problem, not a conviction problem, and re-testing another ETF doesn't fix it.
    - Added 2026-08-08 after XLI/XLB failed R:R identically for 5 consecutive sessions (Aug 3-6) while 7 scanner-tracked sectors were never attempted.
